...
source_eclairage %>% 
  group_by(Q1_SOURCE_ECLAIRAGE_SINGLE_FCT) %>% 
  summarise(duree = mean(Q1_DUREE_UTILISATION_JOUR_NUM*60, na.rm=TRUE)%>%  round(2) %>% as_hms())

# Coût mensuel
source_eclairage %>% 
  group_by(Q1_SOURCE_ECLAIRAGE_SINGLE_FCT) %>% 
  summarise(cout = mean(Q1_COUT_MENSUEL_CONSOMMATION_NUM, na.rm=TRUE)%>%  round(2)) %>% 
  arrange(desc(cout))
